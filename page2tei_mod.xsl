...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="rs" select="false()" />
...
<TEI>
   <teiHeader xml:lang="en">
      <fileDesc>
         <titleStmt>
            ...
            <principal><xsl:comment>Name of the main person responsible for the digital edition</xsl:comment></principal>
            <xsl:comment>Inserire tanti &lt;sponsor&gt; o &lt;funder&gt; quanti ce ne sono nel progetto</xsl:comment>
            <sponsor><xsl:comment>Istituto sponsor del progetto</xsl:comment></sponsor>
            <funder><xsl:comment>Persona, istituto, o contributo che ha finanziato il progetto</xsl:comment></funder>
            <xsl:comment>Inserire tanti &lt;respStmt&gt; a seconda di quanti sono i curatori</xsl:comment>
            <respStmt>
               <name><xsl:comment>Nome del curatore</xsl:comment></name>
               <xsl:comment>Inserire tanti &lt;resp&gt; a seconda delle responsabilità assunte dal curatore: segmentazione, trascrizione, revisione, codifica_tk, codifica_xml, postproduzione, publicazione</xsl:comment>
               <resp><xsl:comment>Tipo di responsabilità secondo il vocabolario definito</xsl:comment></resp>
            </respStmt>
         </titleStmt>
         <editionStmt>
         ...
         <publicationStmt>
           <publisher><xsl:comment>Fornire il nome dell'istituto responsabile per la pubblicazione</xsl:comment></publisher>
            <date when="yyyy-mm-dd"><xsl:comment>Data di pubblicazione: yyyy-mm-dd</xsl:comment></date>
            <availability>
            <licence target="URL"><xsl:comment>Tipo di licenza (es. CC BY-SA)</xsl:comment></licence></availability>
            <idno type="DOI"><xsl:comment>Inserire DOI</xsl:comment></idno>
            <idno type="ID"><xsl:comment>Inserire identificativo volume</xsl:comment></idno>
         </publicationStmt>
         <seriesStmt>
           <title>Enciclopedia delle Antichità</title>
            <biblScope unit="volume" n="15"></biblScope>
            <biblScope unit="libro" n="XVII"></biblScope>
         </seriesStmt>
         <notesStmt>
           <xsl:comment>Inserire tanti &lt;note&gt; a seconda delle necessità</xsl:comment>
            <note><ab><xsl:comment>Inserire una nota se necessario per spiegare aspetti particolari del documento</xsl:comment></ab></note>
         </notesStmt>
         <sourceDesc>
            <msDesc>
             <msIdentifier>
                <settlement>Torino</settlement>
                <repository>Archivio di Stato di Torino</repository>
                <idno type="shelfmark"><xsl:comment>Segnatura del manoscritto: segnatura</xsl:comment></idno>
             </msIdentifier>
             <msContents>
               <ab><xsl:comment>Abstract del manoscritto</xsl:comment></ab>
             </msContents>
             <physDesc><ab><xsl:comment>Descrizione fisica del manoscritto</xsl:comment></ab></physDesc>
          </msDesc>
         </sourceDesc>
      </fileDesc>
     <encodingDesc>
        <projectDesc><ab><xsl:comment>Scopo dell’edizione digitale e altre informazioni rilevanti sul processo</xsl:comment></ab></projectDesc>
        <editorialDecl><ab><xsl:comment>Dettagli dei principi editoriali e pratiche applicate durante la codifica</xsl:comment></ab></editorialDecl>
        <xsl:comment>Definizione della tassonomia per la descrizione delle immagini</xsl:comment>
       <classDecl>
     ...
     <profileDesc>
        <langUsage>
           ...
           <language ident="gr">Greco</language>
        </langUsage>
     </profileDesc>
     <revisionDesc><xsl:comment>Inserire un &lt;change&gt; per ogni tipo o gruppo di cambiamenti fatti al documento dopo l'esportazione da Transkribus</xsl:comment>
        <change><xsl:comment>Inserire una descrizione dei cambiamenti fatti specificando @when e @who</xsl:comment></change>
     </revisionDesc>
   </teiHeader>
   <facsimile
   ...
   <text>
      ...
      <xsl:for-each-group select="$make_div//*[local-name() = 'div']/*" group-starting-with="*[local-name() = 'head']" >
         <div type="" xmlns="http://www.tei-c.org/ns/1.0">
  ...
  <xsl:template match="mets:amdSec" mode="titleStmt">
    <xsl:apply-templates select="descendant::trpDocMetadata/title" /><xsl:value-of select="$nl"/>
   ...
   <xsl:template match="p:TextRegion" mode="text">
      ...
      <xsl:choose>
         ...
         <xsl:when test="'caption' = (@type, $custom?structure?type)">
            <figure>
              <figDesc></figDesc>
               <ab facs="#facs_{$numCurr}_{@id}"><xsl:apply-templates select="p:TextLine" /></ab>
            </figure>
         </xsl:when>
         <xsl:when test="'header' = (@type, $custom?structure?type)" xml:space="preserve">
            <fw type="running-head" place="top" facs="#facs_{$numCurr}_{@id}"><xsl:apply-templates select="p:TextLine"
            /></fw></xsl:when>
    ...
    <xsl:choose>
      ...
      <xsl:when test="@type = 'person'">
        ...
        <xsl:element name="{$elName}">
          ...
          <xsl:if test="$custom('ref') != ''">
            <xsl:attribute name="ref"><xsl:value-of select="$custom('ref')"/></xsl:attribute>
          </xsl:if>
